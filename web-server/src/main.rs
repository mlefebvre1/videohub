...
use videohub::{
    protocol::{
        Configuration, DeviceInfo, HubInfo, InputLabels, OutputLabels, OutputLocks, OutputRoutings,
    },
    Hub,
};
...
fn input_labels_get() -> RequestResult<InputLabels> {
    let hub_info = read_hub_info()?;
    Ok(Json(hub_info.input_labels))
}

// #[put("/input_labels", format = "json", data = "<labels>")]
// fn input_labels_put(labels: Json<InputLabels>) -> RequestResult<Vec<Label>> {
//     let _ = write_hub_info(BlockType::InputLabels(InputLabels(&labels)))?;
//     Ok(labels)
// }

#[get("/output_labels", format = "json")]
fn output_labels_get() -> RequestResult<OutputLabels> {
    let hub_info = read_hub_info()?;
...
fn video_output_locks_get() -> RequestResult<OutputLocks> {
    let hub_info = read_hub_info()?;
...
fn video_output_routing_get() -> RequestResult<OutputRoutings> {
    let hub_info = read_hub_info()?;
